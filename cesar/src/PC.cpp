...
void PC::FS()
{
    ...
    {
        case 1:// MOV
            if(po->mem->memoria[po->regs->read(7) + 1] > 127 and po->mem->memoria[po->regs->read(7) + 1] < 256)//Registrador para memoria
            {
                po->mem->write(po->mem->memoria[po->regs->read(7) + 1], po->regs->read(po->mem->memoria[po->regs->read(7) + 2]));
                po->regs->inc_pc(3);
            }else //Memoria para registrador
            {
